...
- **markdown 뷰 자체도 엔진에 위임 + 폴백**: 다이어그램·PlantUML 위임과 같은 원리를 문서 전체 렌더링으로 확장 — `GraphEngine::render_document(src, width) -> Option<Rendered>`(기본 `None`)를 `DgEngine`만 `dg::render_markdown`으로 구현. 실제 화면용 진입점 `markdown::render_for_display`만 이걸 거치고, `markdown::render`/`render_with`(앱/UI 테스트 수십 개가 엔진과 무관한 결정론적 픽스처로 직접 호출)는 그대로 둬 두 경로를 분리했다. `dg::Style`↔`ratatui::style::Style` 필드 대 필드 변환(`SpanStyle::Raw`)이라 무손실이고, dg가 안 주는 헤딩/각주 메타데이터는 `markdown::delegated`가 별도의 가벼운 pulldown-cmark 스캔으로 재구성해 텍스트 일치로 줄 번호를 되찾는다. 자세한 설계는 "markdown — Renderer" 절.
...
```rust
pub fn render(src: &str, width: u16) -> Rendered;                        // Theme::default()로 render_with를 감싼 얇은 래퍼 — 엔진 무관, 순수
pub fn render_with(src: &str, width: u16, theme: &Theme) -> Rendered;    // theme.heading_rule만 구조에 반영 — 위와 동일하게 엔진 무관
pub fn render_for_display(src: &str, width: u16) -> Rendered;           // 실제 화면용 진입점 — 아래 "위임 + 폴백" 참고
...
pub enum SpanStyle { Plain, Bold, Italic, Strikethrough, Code, Link, Raw(ratatui::style::Style) }   // Raw: 위임 렌더러가 이미 확정한 스타일 그대로
...
- **markdown 전체를 엔진에 위임 + 폴백 (2026-09-22 개선, `markdown::delegated`)**: `render`/`render_with`는 일부러 손대지 않았다 — `app`/`ui` 리듀서 테스트 수십 개가 이 둘을 "무엇을 테스트하든 상관없는 결정론적 픽스처 빌더"로 직접 호출하므로, 전역 엔진 선택 상태에 결과가 좌우되면 그 테스트들이 서로 무관한 이유로 깨지게 된다. 대신 실제 화면에 걸리는 문서(`app::loader::load_doc`/`load_definition`, 이 둘만)는 새 진입점 `render_for_display(src, width)`를 거친다 — 선택된 `GraphEngine::render_document(src, width) -> Option<Rendered>`(기본 `None`)를 먼저 묻고, `Some`이면 그대로 쓰고 `None`이면 `render(src, width)`로 되돌아간다. `DgEngine`은 이를 `dg::render_markdown`으로 구현 — 표·목록·인용·코드펜스(다이어그램 포함)까지 dg 자신이 통째로 그린 `Vec<dg::Line>`을 받아, `(text, dg::Style)` 구간마다 그대로 `Span{ style: SpanStyle::Raw(ratatui 변환) }`로 옮긴다(`dg::Style`과 `ratatui::style::Style`이 둘 다 "인덱스 색 + bold/dim/italic/underline/strike/reverse 플래그" 구조라 필드 대 필드 변환, 손실 없음 — `ui::doc_panel`이 자기 6종 `SpanStyle` 팔레트로 다시 매핑해야 했던 기존 경로와 다르다). dg는 `HeadingAnchor`/`Footnote` 목록을 안 주므로, 같은 소스를 `markdown::delegated::scan_headings_and_footnotes`(가벼운 pulldown-cmark 스캔, 렌더링과 무관)로 따로 훑어 (레벨, 텍스트) 순서를 얻고, `locate_heading_lines`가 dg가 만든 `plain` 줄들 중 그 텍스트와 정확히 일치하는 첫 줄(커서 전진, 중복 제목 구분)을 찾아 `HeadingAnchor.line`을 매긴다 — 못 찾으면(제목이 길어 dg가 줄바꿈한 경우 등) 조용히 건너뛴다(TOC 항목 하나 빠지는 게 엉뚱한 곳으로 점프하는 것보다 안전). 각주는 정의 순서로 1부터 번호만 매긴다(`Rendered.footnotes`는 실제로 앱 어디서도 안 읽는 테스트 전용 필드라 첫 참조 순서까지 재현할 필요는 없다고 판단). `mdview`/`builtin` 선택 시엔 `render_document`가 기본값 `None`이라 `render_for_display`가 그대로 `render`로 떨어져, 기존 자체 렌더러 동작이 완전히 보존된다.
...
- **Mouse/Visual** (`tests/visual_defects.rs`, `TestBackend` 실버퍼 검사): 클릭/드래그/스크롤이 실제 렌더 셀에 반영되는지 — 이 중 `.kiro/specs/spec-viewer/design.md`(이 fixture가 아니라, 아래 설명하는 이 크레이트 자신의 실제 `.kiro` 사양)의 `## 정의` 문단을 대상으로 한 3개는 표시 폭 클리핑 회귀 검증. `tests/table_overflow.rs`도 같은 실제 design.md로 표 오버플로 회귀를 검증한다. 이 크레이트는 2026-09-22(`0ab79a3`)부터 자기 `.kiro/specs/*`(spec-viewer 본편 + bugfix 스펙 5개: files-tree-scroll-latency/fs-event-targeted-update/kiro-mode-freeze/mouse-scroll-freeze/watch-startup-latency)를 저장소 안에 직접 들고 있다(예전엔 `../.kiro`로 한 단계 위 워크스페이스를 가정해 독립 체크아웃 시 이 6개 테스트가 `DocView::Missing`으로 깨졌었다 — 이제 경로가 `CARGO_MANIFEST_DIR/.kiro`로 바뀌어 항상 통과한다).
